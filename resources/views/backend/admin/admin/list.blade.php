      ...
      <div class="row align-items-end justify-content-between flex-lg-nowrap">
         <div class="col-12 col-lg-auto col-xl-auto flex-grow-1 mb-3 mb-lg-0">
            <div class="title-and-status d-md-flex justify-content-between justify-content-lg-start align-items-center">
               <h3 class="page-title breadcrumb-trail">{{ __('admin.Admin_List') }}</h3>
               <div class="status-search">
                  <p class="status-text">
                     <a href="{{ url('admin/admin/add') }}" style="margin-left: 10px;" class="btn btn-danger">{{ __('admin.Add_new_Admin') }}</a>
                  </p>
               </div>
            </div>
         </div>
               ...
               <form action="" method="get" class="multi-search-form d-flex align-items-end justify-content-end">
                  <div class="input-group">
                     <div class="form-group">
                        <label for="lang-multi">{{ __('admin.Admin_ID') }}</label>
                        <input type="text" name="id" value="{{ Request()->id }}" class="form-control" style="height: 35px;" placeholder="{{ __('admin.Enter_Admin_ID') }}">
                     </div>
                     ...
                     <div class="form-group">
                        <label for="lang-multi">{{ __('admin.Admin_name') }}</label>
                  ...
                  <i class="fas fa-search"></i>
                     ...
                     <a href="{{ url('admin/admin') }}" class="multi-search-submit d-inline-flex align-items-center justify-content-between" style="margin-left: 10px;color: #fff;">{{ __('admin.Reset') }}</a>
                           ...
                           <tbody>
                              ...
                              <tr class="single-course-item">
                                <td>{{ $value->id }}</td>
                                 ...
                                 <td class="earning" data-title="Earning">{{ $value->email }}</td>
                                 <td>
                                    
                                       @if($value->is_admin == 1)
                                          <span>Admin</span>
                                       @elseif($value->is_admin == 4) 
                                 ...
                                 <td class="action" data-title="Action">
                                    ...
                                    <a href="{{ url('admin/admin/edit/'.$value->id) }}" class="button error-btn">
                                    <i class="fas fa-edit"></i>
                                    </a>
                                  
                                    <a onclick="delete_record('{{ url('admin/admin/delete/'.$value->id) }}')" class="button trash-btn"><i class="far fa-trash-alt"></i></a>

                                    @if (Auth::user()->is_admin == "4")
                                        <a href="{{ url('admin/admin/permission/'.$value->id) }}" class="btn btn-primary">Permission</a>
...
<script>
   $(document).ready(function(){
         $('.ChangeRequestStatus').change(function(){
               ...
               $.ajax({
                     type: 'GET',
                     url:"{{ url('admin/admin/change_admin_status') }}",
