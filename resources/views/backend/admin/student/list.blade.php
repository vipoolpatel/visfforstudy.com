      ...
      <div class="row align-items-end justify-content-between flex-lg-nowrap">
         <div class="col-12 col-lg-auto col-xl-auto flex-grow-1 mb-3 mb-lg-0">
            <div class="title-and-status d-md-flex justify-content-between justify-content-lg-start align-items-center">
               <h3 class="page-title breadcrumb-trail">{{ __('admin.Student_List') }}</h3>
               <div class="status-search">
                  <p class="status-text">
                     <a href="{{ url('admin/student/add') }}" style="margin-left: 10px;" class="btn btn-danger">{{ __('admin.Add_new_Student') }}</a>
                  </p>
               </div>
            </div>
         </div>
               ...
               <form action="" method="get" class="multi-search-form d-flex align-items-end justify-content-end">
                  <div class="input-group">
                    <div class="form-group">
                        <label for="lang-multi">{{ __('admin.Student_ID') }}</label>
                        <input type="text" name="id" value="{{ Request()->id }}" class="form-control" style="height: 35px;" placeholder="{{ __('admin.Enter_Student_ID') }}">
                     </div>
                     <div class="form-group">
                  ...
                  <a href="{{ url('admin/student') }}" class="multi-search-submit d-inline-flex align-items-center justify-content-between" style="margin-left: 10px;color: #fff;">{{ __('admin.Reset') }}</a>
               </form>
            </div>
         </div>
      </div>
                           ...
                           <tbody>
                              ...
                              <tr class="single-course-item">
                                 ...
                                 <td class="action" data-title="Action">
                                    ...
                                    <a href="{{ url('admin/student/view/'.$value->id) }}" class="button view-btn"><i class="far fa-eye"></i></a>
                                    <a href="{{ url('admin/student/edit/'.$value->id) }}" class="button error-btn">
                                    <i class="fas fa-edit"></i>
                                    </a>
                                    <a href="{{ url('admin/send_mssage/'.$value->id) }}" class="button view-btn">Chat</a>

                                    <a onclick="delete_record('{{ url('admin/student/delete/'.$value->id) }}')" class="button trash-btn"><i class="far fa-trash-alt"></i></a>
...
<script>
   $(document).ready(function(){
         $('.ChangeRequestStatus').change(function(){
               ...
               $.ajax({
                     type: 'GET',
                     url:"{{ url('admin/student/change_student_status') }}",
