      ...
      <div class="row align-items-lg-center h-100">
         ...
         <div class="col-12 col-lg-11 offset-lg-1 h-100">
            ...
            <div class="hero-main-content">
               <h2 class="hero-title text-capitalize">{{ __('tutor.Edit_your_offer') }}</h2>
                     ...
                     <div class="find-multi-search-box booking-form-container">
                        <form action="" method="post" class="booking-form offer-form">
                           ...
                           <div class="form-group title-field">
                              ...
                              <select disabled style="max-width: 700px !important;" required name="student_id" class="form-control">
                                 <option value="">{{ __('tutor.Select_Student') }}</option>
                                 @foreach($getStudent as $value_s)
                                    <option {{ ($value->student_id == $value_s->id) ?  'selected' : ''  }} value="{{ $value_s->id }}">{{ $value_s->name }} {{ $value_s->last_name }}</option>
                                 @endforeach
                              </select>
                           </div>
                           ...
                           <div class="form-group title-field">
                              ...
                              <input type="text" name="title" required class="offer-title form-control" placeholder="{{ __('tutor.Enter_your_offer_title') }}" value="{{ $value->title }}">
                           </div>
...
{{ __('tutor.Select_What_Subject_You_Want_to_teach') }}
                                 ...
                                 <div class="form-group">
                                    <label for="offer-level-multi" style="font-size: 18px;">
{{ __('tutor.Whats_Your_Level') }}
                                    </label>
                                    <select name="level_id" id="offer-level-multi" required class="level-multi form-control">
                                       <option value="">
{{ __('tutor.Select_Whats_Your_Level') }}
                                      </option>
                                       @foreach($getLevelOfStudent as $value_c) 
                                          <option {{ ($value->level_id == $value_c->id) ?  'selected' : ''  }} value="{{ $value_c->id }}">{{ $value_c->getlevelofstudentname() }}</option>
                                       @endforeach
                                    </select>
                                 </div>
                                 <div class="form-group">
                                    <label for="offer-lang-multi" style="font-size: 18px;">
                                    {{ __('tutor.Lesson_language') }}
                                    </label>
                                    <select name="language_id" id="offer-lang-multi" required class="offer-lang-multi form-control">
                                       <option value="">
                                       {{ __('tutor.Select_Lesson_Language') }}
                                       </option>
                                       @foreach($getLanguage as $value_l) 
                                          <option {{ ($value->language_id == $value_l->id) ?  'selected' : ''  }} value="{{ $value_l->id }}">{{ $value_l->getlanguagename() }}</option>
                                       @endforeach
                                    </select>
                                 </div>
                           ...
                           <div class="form-group title-field">
                                 <label>
                                 {{ __('tutor.Price_For_Each_Lesson') }}
                               </label>
                                 <input type="text" name="lesson_per_rate" value="{{ $value->lesson_per_rate }}" required class="publish-course-title form-control" placeholder="{{ __('tutor.Price_For_Each_Lesson') }}">
                           </div>
                           ...
                           <div class="book-form-check custom">
                              <h5 class="label">{{ __('tutor.What_type_of_lesson_do_you_Provide') }}</h5>
                                 ...
                                 <div class="form-group d-inline-flex align-items-center mb-0 mr-4">
                                    <input type="radio"  {{ ($value->lesson_type_id == $type->id) ?  'checked' : ''  }} name="lesson_type_id"  required id="getRequestType{{ $type->id }}" class="reg-check form-check-input" value="{{ $type->id }}">
                           ...
                           <div class="time-picker">
                              <div class="form-group">
                                 ...
                                 <div class="form-check-inline">
                                    <input type="date" value="{{ $value->start_date }}" required name="lesson_date" class="form-control">

                                    <span class="time-picker-separator">&amp;</span>
                                    
                                    <input type="time" required value="{{ $value->start_time }}" style="padding: 0px;padding-left: 10px;" name="lesson_time" class="form-control">

                                    <span class="time-picker-separator">&amp;</span>

                                    <input type="text" required value="{{ $value->duration }}" name="duration" class="form-control" placeholder="{{ __('tutor.Duration_Minutes') }}">
                           ...
                           <div class="text-message">
                              <div class="form-group">
                                 ...
                                 <textarea name="description" id="offer-message" class="booking-message offer-message form-control" placeholder="{{ __('tutor.Type_a_Description') }}" cols="10" rows="5">{{ $value->description }}</textarea>
                              </div>
                           </div>

                           <div class="offer-submit-btn-cont">
                              <button type="submit" class="offer-submit-btn reg-signup-btn text-capitalize">{{ __('tutor.Update_offer') }}</button>
                           </div>
                        </form>                          
                     </div>
                  </div>
